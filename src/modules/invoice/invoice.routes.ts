...
const { create, createFromAppointment, index, pay, payments, patientInvoices, visitInvoices, patientPayments } = require('./invoice.controller');
...
router.get('/patients/:patientId', authenticate(), authorize(ACCESS_GROUPS.BILLING_MANAGERS), patientInvoices);
router.get('/patients/:patientId/payments', authenticate(), authorize(ACCESS_GROUPS.BILLING_MANAGERS), patientPayments);
router.get('/visits/:visitId', authenticate(), authorize(ACCESS_GROUPS.BILLING_MANAGERS), visitInvoices);
